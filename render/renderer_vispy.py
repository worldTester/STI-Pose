...
class RendererVispy(Renderer, app.Canvas, metaclass=SingletonArgs):
  """A Python based renderer."""

  def __init__(self, width, height, mode="mask", shading="phong", bg_color=(0.0, 0.0, 0.0, 0.0)):
    """Constructor.
    ...
    """
    Renderer.__init__(self, width=width, height=height)
    ...
    self.render_rgb = (self.mode == 'rgb')
    self.render_mask = (self.mode == 'mask')


    # Structures to store object models and related info.
    self.models = {}
    self.deltaP = None # P_origin_reset
  ...
  def add_object(self, obj_id, model_path, **kwargs):
    ...
    self.pts = model["pts"].copy()
    center = np.mean(model["pts"],axis=0)
    model["pts"]-=center
  ...
  def on_draw(self, obj_id, mat_model, mat_view, mat_proj, clear=True):
    with self.fbo:
      ...
      if self.render_mask:
        self.mask = self._draw_rgb(obj_id, mat_model, mat_view, mat_proj)
